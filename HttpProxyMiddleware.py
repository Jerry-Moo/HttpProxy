...
class HttpProxyMiddleware(object):
    # 遇到这类型的错误直接当做代理不可用处理，不再传给retrymiddleware
    ...
    def __init__(self, settings):
        ...
        # 存放代理列表的文件，每行一个代理， 格式为ip:port, 注意是没有http://，而且这个文件是会被修改，注意备份
        self.proxy_file = 'proxyes.dat'
        # 是否在超时的情况下禁用代理
        self.invalid_proxy_flag = True
        # 当有效代理小于这个数值时（包括直连），从网上抓取新的代理，可以将这个数设为为了满足每个ip被要求输入验证码后得到足够休息时间所需要的代理数
        # 例如爬虫在10个可用代理之间切换时，每个ip经过数分钟才再一次轮到自己，这样就能get到一些请求而不用输入验证码
        # 如果这个数过小，例如2个，爬虫用A-ip爬了没几个就被ban，换了一个B-ip又爬了没几次就被ban，这样整个爬虫就处于一种忙等待的状态，影响效率
        ...
        for np in new_proxyes:
            if self.url_in_proxyes('http://' + np):
                ...
                self.proxyes.append({'proxy': 'http://' + np,
                                     'valid': True,
                                     'count': 0})
        # 如果发现抓不到什么新的代理，缩小threshold以避免白费功夫
    ...
    def inc_proxy_index(self):
        ...
        # 两轮proxy_index==0的时间间隔果断，说明出现了验证码抖动，扩展代理列表
